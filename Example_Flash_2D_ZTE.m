%% Zero Gradient Echo Time 2D (ZTE 2D)
% This example acquires a 2D zero gradient echo time (ZTE) image with Ernst angle excitation.

%%
LoadSystem;                                         % load system parameters (reset to default: HW Seq AQ TX Grad)

[HW, mySave] = Find_Frequency_Sweep(HW, mySave, 0);

% HW.TX2RXdeadTime = 20e-6;

Seq.Loops = 1;                                      % number of loop averages 1...

Seq.T1 = 100e-3;                                    % T1 of sample; excitation angle is acos(exp(-Seq.tRep/Seq.T1))/pi*180
Seq.tEcho = 0e-3;                                   % echo time in seconds e.g. 4e-3
Seq.tRep = 5e-3;                                    % repetition time in seconds (default is Seq.tEcho*2)

% % Pixels and size %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Seq.AQSlice(1).nRead = 64;                          % number of "pixels" in read direction
Seq.AQSlice(1).ReadOS = 8;                          % number of "pixels" in read direction
Seq.AQSlice(1).HzPerPixMin = 300;                   % bandwidth per pixel in Hz (1/HzPerPixMin = duration of AQ window, 0: longest possible)
% "phase" direction are for k-space end points and image reconstruction
Seq.AQSlice(1).nPhase(1) = 40;                      % number of pixels in phase(1) direction
Seq.AQSlice(1).nPhase(2) = 40;                      % number of pixels in phase(2) direction
% Seq.AQSlice(1).nPhase(3) = 40;                      % number of pixels in phase(3) direction
Seq.AQSlice(1).sizePhase(1) = 0.012;                % size in phase(1) direction in meter
Seq.AQSlice(1).sizePhase(2) = 0.012;                % size in phase(2) direction in meter
Seq.AQSlice(1).sizePhase(3) = 0.012;                % size in phase(3) direction in meter
% Seq.AQSlice(1).thickness = 0.020;                   % slice thickness in meter
Seq.AQSlice(1).excitationPulse = @Pulse_RaisedCos;  % excitation pulse function (type "Pulse_" than press tab for selection of pulses)

% Seq.AQSlice(1).kLineOrderType = 'centerout';

% % Oversampling %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Seq.AQSlice(1).PhaseOS(1) = 1;                      % oversampling phase(2)  1...
Seq.AQSlice(1).PhaseOS(2) = 1;                      % oversampling phase(2)  1...
% Seq.AQSlice(1).PhaseOS(3) = 2;                      % oversampling phase(2)  1...

% % Orientation in space %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
orientation = 'zx';                                 % 'xy', 'yz', 'zx' for one of the cardinal planes (read-phase)
switch orientation
  case 'xy'
    Seq.AQSlice(1).alfa = 1.0*pi;                   % 1st rotation around x axis in RAD
    Seq.AQSlice(1).phi  = 0.0*pi;                   % 2nd rotation around y axis in RAD
    Seq.AQSlice(1).theta= 0.5*pi;                   % 3rd rotation around z axis in RAD
  case 'yz'
    Seq.AQSlice(1).alfa = 0.5*pi;                   % 1st rotation around x axis in RAD
    Seq.AQSlice(1).phi  = 0.0*pi;                   % 2nd rotation around y axis in RAD
    Seq.AQSlice(1).theta= 0.5*pi;                   % 3rd rotation around z axis in RAD
  case 'zx'
    Seq.AQSlice(1).alfa = 0.5*pi;                   % 1st rotation around x axis in RAD
    Seq.AQSlice(1).phi  = 0.0*pi;                   % 2nd rotation around y axis in RAD
    Seq.AQSlice(1).theta= 0.0*pi;                  % 3rd rotation around z axis in RAD
  otherwise
    if ~ischar(orientation)
      orientation = num2str(orientation);
    end
    error('Unknown orientation "%s"\n', orientation);
end
% Seq.AQSlice(1).phi = Seq.AQSlice(1).phi + 0.5*pi;  % un-comment to exchange read and phase direction

Seq.SteadyState_PreShots = 5;

% % Plot %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Seq.plotSeq = 1:3;                                  % plot sequence on real timeline, plots RF, AQ and Grad (1==x, 2==y, 3==z, 0 no gradient)
% Seq.plotSeqAQ = 1:3;                                % plot sequence wrapped after echo gradient echo (train), plots RF, AQ and Grad (1==x, 2==y, 3==z, 0 no gradient)
Seq.LoopPlot = 1;                                   % plot every loop
Seq.AQSlice(1).plotkSpace = 1;                      % plot k-space
Seq.AQSlice(1).plotImage = 1;                       % plot image
Seq.AQSlice(1).plotPhase = 1;                       % plot phase of k-space and/or image
Seq.AQSlice(1).ZeroFillFactor = 1;                  % zero fill resolution factor

Seq.CorrectSliceRephase = 0;                        % Correct SliceGradTimeIntegralOffset
Seq.CorrectReadRephase = 0;                         % Correct ReadGradTimeIntegralOffset
Seq.CorrectPhase = 1;                               % use frequency tracking windows and correct the phase of the encoded signal
Seq.CorrectPhaseDuration = 2e-3;                    % duration of frequency tracking windows in s

Seq.plotZTEEndPoints = true;
Seq.AQSlice(1).NUFFTReconstruction = 1;             % use non-uniform Fourier transform for image reconstruction
Seq.AQSlice(1).ZeroFillWindowSize = 1;              % zero fill window size (high k-space values are damped by a cos^2 law)

[SeqLoop, mySave] = sequence_Flash(HW, Seq, AQ, TX, Grad, mySave);

if 0
  %% Test zero fill and window
  SeqLoop.AQSlice(1).ZeroFillWindowSize = 1;          % zero fill window size (high k-space values are damped by a cos^2 law)
  SeqLoop.AQSlice(1).ZeroFillFactor = 1;              % zero fill resolution factor
  SeqLoop.data.RoI = [];
  SeqLoop.AQSlice(1).NUFFTReconstruction = 1;
  % SeqLoop.AQSlice(1).plotkSpace = 1;                  % plot k-space
  % SeqLoop.AQSlice(1).plotkSpacePhase = 1;             % plot phase of k-space
  % SeqLoop.data.kSpaceOs = [];
  [SeqLoop.data] = get_kSpaceAndImage(SeqLoop.data, SeqLoop.AQSlice(1));
  [SeqLoop.data, SeqLoop.AQSlice] = plot_kSpaceAndImage(SeqLoop.data, SeqLoop.AQSlice(1));
end

%% -----------------------------------------------------------------------------
% (C) Copyright 2021-2025 Pure Devices GmbH, Wuerzburg, Germany
% www.pure-devices.com
%-------------------------------------------------------------------------------
