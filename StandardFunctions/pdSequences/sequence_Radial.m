...
if ~isfield(Seq.AQSlice,'HzPerPixMin');Seq.AQSlice(1).HzPerPixMin=[];end;if isempty(Seq.AQSlice(1).HzPerPixMin); Seq.AQSlice(1).HzPerPixMin=500;end
if ~isfield(Seq,'Loops');Seq.Loops=[];end;if isempty(Seq.Loops); Seq.Loops=1;end
if ~isfield(Seq,'LoopsBreak');Seq.LoopsBreak=[];end;if isempty(Seq.LoopsBreak); Seq.LoopsBreak=[];end
if ~isfield(Seq,'LoopsBreakExactly');Seq.LoopsBreakExactly=[];end;if isempty(Seq.LoopsBreakExactly); Seq.LoopsBreakExactly=0;end
if ~isfield(Seq,'LoopPlot');Seq.LoopPlot=[];end;if isempty(Seq.LoopPlot); Seq.LoopPlot=1;end
if ~isfield(Seq,'tRep');Seq.tRep=[];end;if isempty(Seq.tRep); Seq.tRep=1/Seq.AQSlice(1).HzPerPixMin*2.5+1e-3;end
if ~isfield(Seq,'average');Seq.average=[];end;if isempty(Seq.average); Seq.average=1;end
if ~isfield(Seq,'T1');Seq.T1=[];end;if isempty(Seq.T1); Seq.T1=100e-3;end
...
if ~isfield(Seq,'SteadyState_PostShots');Seq.SteadyState_PostShots=[];end;if isempty(Seq.SteadyState_PostShots); Seq.SteadyState_PostShots=Seq.SteadyState_PreShots;end
if ~isfield(Seq,'CorrectPhase');Seq.CorrectPhase=[];end;if isempty(Seq.CorrectPhase); Seq.CorrectPhase=0;end
if ~isfield(Seq,'CorrectPlotFrequency');Seq.CorrectPlotFrequency=[];end;if isempty(Seq.CorrectPlotFrequency); Seq.CorrectPlotFrequency=Seq.CorrectPhase;end
if ~isfield(Seq,'StartWithKSpaceCenterNonlinear');Seq.StartWithKSpaceCenterNonlinear=[];end;if isempty(Seq.StartWithKSpaceCenterNonlinear); Seq.StartWithKSpaceCenterNonlinear=0;end
if ~isfield(Seq,'StartWithGradientBeforeExcitationPulse');Seq.StartWithGradientBeforeExcitationPulse=[];end;if isempty(Seq.StartWithGradientBeforeExcitationPulse); Seq.StartWithGradientBeforeExcitationPulse=0;end
if ~isfield(Seq,'AQSlice');Seq.AQSlice=[];end;
if ~isfield(Seq.AQSlice,'ReadRadial');Seq.AQSlice(1).ReadRadial=[];end;if isempty(Seq.AQSlice(1).ReadRadial); Seq.AQSlice(1).ReadRadial=1;end
if ~isfield(Seq.AQSlice,'FixedReadGrad');Seq.AQSlice(1).FixedReadGrad=[];end;if isempty(Seq.AQSlice(1).FixedReadGrad); Seq.AQSlice(1).FixedReadGrad=1;end
if ~isfield(Seq.AQSlice,'ReadCoordinate');Seq.AQSlice(1).ReadCoordinate=[];end;if isempty(Seq.AQSlice(1).ReadCoordinate); Seq.AQSlice(1).ReadCoordinate=1;end
if ~isfield(Seq.AQSlice,'PhaseCoordinate');Seq.AQSlice(1).PhaseCoordinate=[];end;if isempty(Seq.AQSlice(1).PhaseCoordinate); Seq.AQSlice(1).PhaseCoordinate=[1 2 3];end
if ~isfield(Seq.AQSlice,'SliceCoordinate');Seq.AQSlice(1).SliceCoordinate=[];end;if isempty(Seq.AQSlice(1).SliceCoordinate); Seq.AQSlice(1).SliceCoordinate=3;end
if ~isfield(Seq.AQSlice,'plotImageHandle');Seq.AQSlice(1).plotImageHandle=[];end;if isempty(Seq.AQSlice(1).plotImageHandle); Seq.AQSlice(1).plotImageHandle=110;end
if ~isfield(Seq.AQSlice,'UseAQWindow');Seq.AQSlice(1).UseAQWindow=[];end;if isempty(Seq.AQSlice(1).UseAQWindow); Seq.AQSlice(1).UseAQWindow=1+(Seq.CorrectPhase>0);end
if ~isfield(Seq.AQSlice,'PhaseOS');Seq.AQSlice(1).PhaseOS=[];end;if isempty(Seq.AQSlice(1).PhaseOS); Seq.AQSlice(1).PhaseOS=[1 1 1];end
if ~isfield(Seq.AQSlice,'sizePhase');Seq.AQSlice(1).sizePhase=[];end;if isempty(Seq.AQSlice(1).sizePhase); Seq.AQSlice(1).sizePhase=HW.Grad.ImageVol(2:2:6)-HW.Grad.ImageVol(1:2:6);end
if ~isfield(Seq.AQSlice,'sizeRead');Seq.AQSlice(1).sizeRead=[];end;if isempty(Seq.AQSlice(1).sizeRead); Seq.AQSlice(1).sizeRead=HW.Grad.ImageVol(2)-HW.Grad.ImageVol(1);end
if ~isfield(Seq.AQSlice,'thickness');Seq.AQSlice(1).thickness=[];end;if isempty(Seq.AQSlice(1).thickness); Seq.AQSlice(1).thickness=1e12;end
if ~isfield(Seq.AQSlice,'plotPhase');Seq.AQSlice(1).plotPhase=[];end;if isempty(Seq.AQSlice(1).plotPhase); Seq.AQSlice(1).plotPhase=1;end
if ~isfield(Seq.AQSlice,'plotkSpace');Seq.AQSlice(1).plotkSpace=[];end;if isempty(Seq.AQSlice(1).plotkSpace); Seq.AQSlice(1).plotkSpace=1;end
if ~isfield(Seq.AQSlice,'plotImage');Seq.AQSlice(1).plotImage=[];end;if isempty(Seq.AQSlice(1).plotImage); Seq.AQSlice(1).plotImage=1;end
if ~isfield(Seq.AQSlice,'alfa');Seq.AQSlice(1).alfa=[];end;if isempty(Seq.AQSlice(1).alfa); Seq.AQSlice(1).alfa=0;end
if ~isfield(Seq.AQSlice,'phi');Seq.AQSlice(1).phi=[];end;if isempty(Seq.AQSlice(1).phi); Seq.AQSlice(1).phi=0;end
if ~isfield(Seq.AQSlice,'theta');Seq.AQSlice(1).theta=[];end;if isempty(Seq.AQSlice(1).theta); Seq.AQSlice(1).theta=0;end
if ~isfield(Seq.AQSlice,'nRead');Seq.AQSlice(1).nRead=[];end;if isempty(Seq.AQSlice(1).nRead); Seq.AQSlice(1).nRead=32;end
if ~isfield(Seq.AQSlice,'nPhase');Seq.AQSlice(1).nPhase=[];end;if isempty(Seq.AQSlice(1).nPhase); Seq.AQSlice(1).nPhase=[1 32 1];end
if Seq.AQSlice(1).nPhase(1)==0; Seq.AQSlice(1).nPhase(1)=1;end
if numel(Seq.AQSlice(1).nPhase)==1; Seq.AQSlice(1).nPhase(2)=1;end
if Seq.AQSlice(1).nPhase(2)==0; Seq.AQSlice(1).nPhase(2)=1;end
if numel(Seq.AQSlice(1).nPhase)==2; Seq.AQSlice(1).nPhase(3)=1;end
if Seq.AQSlice(1).nPhase(3)==0; Seq.AQSlice(1).nPhase(3)=1;end
if Seq.AQSlice(1).PhaseOS(1)==0; Seq.AQSlice(1).PhaseOS(1)=1;end
if numel(Seq.AQSlice(1).PhaseOS)==1; Seq.AQSlice(1).PhaseOS(2)=1;end
if Seq.AQSlice(1).PhaseOS(2)==0; Seq.AQSlice(1).PhaseOS(2)=1;end
if numel(Seq.AQSlice(1).PhaseOS)==2; Seq.AQSlice(1).PhaseOS(3)=1;end
if Seq.AQSlice(1).PhaseOS(3)==0; Seq.AQSlice(1).PhaseOS(3)=1;end
if Seq.AQSlice(1).sizePhase(1)==0; Seq.AQSlice(1).sizePhase(1)=HW.Grad.ImageVol(2)-HW.Grad.ImageVol(1);end
if numel(Seq.AQSlice(1).sizePhase)==1; Seq.AQSlice(1).sizePhase(2)=HW.Grad.ImageVol(4)-HW.Grad.ImageVol(3);end
if Seq.AQSlice(1).sizePhase(2)==0; Seq.AQSlice(1).sizePhase(2)=HW.Grad.ImageVol(4)-HW.Grad.ImageVol(3);end
if numel(Seq.AQSlice(1).sizePhase)==2; Seq.AQSlice(1).sizePhase(3)=HW.Grad.ImageVol(6)-HW.Grad.ImageVol(5);end
if Seq.AQSlice(1).sizePhase(3)==0; Seq.AQSlice(1).sizePhase(3)=HW.Grad.ImageVol(6)-HW.Grad.ImageVol(5);end
if ~isfield(Seq,'plotSeqEnd');Seq.plotSeqEnd=[];end;if isempty(Seq.plotSeqEnd); Seq.plotSeqEnd=[];end
if ~isfield(Seq.AQSlice,'ReadOS');Seq.AQSlice(1).ReadOS=[];end;if isempty(Seq.AQSlice(1).ReadOS); Seq.AQSlice(1).ReadOS=max(2,ceil(16000/(Seq.AQSlice(1).HzPerPixMin*Seq.AQSlice(1).nRead)/2)*2);end
if ~isfield(Seq.AQSlice,'ReadOSUsedForImage');Seq.AQSlice(1).ReadOSUsedForImage=[];end;if isempty(Seq.AQSlice(1).ReadOSUsedForImage); Seq.AQSlice(1).ReadOSUsedForImage=min(Seq.AQSlice(1).ReadOS,17);end
if ~isfield(Seq.AQSlice,'excitationPulse');Seq.AQSlice(1).excitationPulse=[];end;if isempty(Seq.AQSlice(1).excitationPulse); Seq.AQSlice(1).excitationPulse=@Pulse_Sinc_1;end
...
if or(Loop==1,Seq.LoopsBreakExactly==0);
...
if Seq.StartWithGradientBeforeExcitationPulse==0;
    ...
    if size(Seq.kgrid.all1,1)==1;Seq.kgrid.selectWall(1,2:end-1,2:end-1)=false;end;
    if size(Seq.kgrid.all1,2)==1;Seq.kgrid.selectWall(2:end-1,1,2:end-1)=false;end;
    if size(Seq.kgrid.all1,3)==1;Seq.kgrid.selectWall(2:end-1,2:end-1,1)=false;end;
end
...
if ~isfield(Seq.AQSlice,'UsetRep');Seq.AQSlice(1).UsetRep=[];end;if isempty(Seq.AQSlice(1).UsetRep); Seq.AQSlice(1).UsetRep=Seq.SteadyState_PreShots+(1:numel(Seq.kgrid.wallr));end
...
if ~isfield(Seq,'tEcho');Seq.tEcho=[];end;if isempty(Seq.tEcho); Seq.tEcho=Seq.Read(1).CenterOfReadout;end
...
if Seq.LoopsBreakExactly
    ...
    elseif and(Loop>=2,Loop~=Seq.Loops);
        Seq.Reinitialize=0; 
...
if isempty(Seq.LoopsBreak); tb=1;else tb=Seq.LoopsBreak;end
if isempty(Seq.averageBreak); ta=0;else ta=Seq.averageBreak;end
if Seq.Loops>1; disp(['Time to run remaining loops ' num2str((sum(Seq.tRep)*Seq.average+ta*(Seq.average-1))*(Seq.Loops-Loop+1)+tb*(Seq.Loops-Loop+1)) ' sec.']);end
clear ta tb
...
if and(~SeqOut.LoopsBreakExactly,~isempty(SeqOut.LoopsBreak))
...
%      if Seq.LoopPlot; [data]=plot_kSpaceAndImage(data,SeqOut.AQSlice(1));end;
     if Loop==1; 
         SeqLoop=SeqOut; 
         SeqLoop.data=data;
     ...
     if Seq.CorrectPhase>0; SeqLoop.dataLoop(Loop).dataWithoutCorrectPhase=data.dataWithoutCorrectPhase; end;
