%% Calibration values for rf amplifier

if ~exist('iDevice', 'var')
  iDevice = 1;
end

HW.TX(iDevice).ExtRFSN = 9999;
HW.TX(iDevice).ExtRFType = 'myRFAmpType';  % string with type identifier
...
HW.TX(iDevice).Def.PaUout(2) = 50;  % default PaUout(2)=50 V => 25 W
...
HW.TX(iDevice).BlankOffset = 1200e-9;  % unblank rf amplifier before TX pulse in s
HW.TX(iDevice).BlankPostset = 400e-9;  % blank rf amplifier after TX pulse in s
...
%%
% UseExtRFAmpSwitch = 0;  % use switch of rf amplifier
