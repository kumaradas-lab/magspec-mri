%% calibration properties and settings for the probe COMPASS #31
...
HW.MPI.ACFrequency_min = 100;  % minimum frequency in Hz
HW.MPI.ACFrequency_max = 100000;  % maximum frequency in Hz
HW.MPI.DC_max = 30e-3;  % maximum DC amplitude in T
HW.MPI.AC_max = 30e-3;  % maximum AC amplitude in T
...
HW.MPI.COMPASS_CoilEfficiency = 2.3e-3;  % efficiency of the TX coil in T/A
...
HW.TX(iDevice).LoadRin(2) = 1.95/(12.39/2);  % resistance of COMPASS TX coil in Ohm
...
if ~isCalibratedCH5
  ...
  I_peak_ACmeas = 4.27/2; % [A]
  I_peak_DCmeas = 4.21/2; % [A]
...
TXACcal=[1,   1000, 4.3/2];
TXACcal=[2,   2000, 4.29/2];
TXACcal=[3,   5000, 4.33/2];
TXACcal=[4,   7500, 4.35/2];
TXACcal=[5,  10000, 4.34/2];
TXACcal=[6,  15000, 4.30/2];
TXACcal=[7,  20000, 4.21/2];
TXACcal=[8,  25000, 4.12/2];
TXACcal=[9,  30000, 3.99/2];
TXACcal=[10, 40000, 3.75/2];
...
massCopper = 20e-3;  % mass of solenoid from simulation
...
HW.TX(iDevice).CoilPowerDissipation = 30;  % thermal power dissipation in W at maximum temperature
