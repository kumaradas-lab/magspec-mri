...
HW.Grad(iDevice).ExtGradSN = 49;                              % serial number
...
HW.Grad(iDevice).PaOffsetI(1:4) = [-0.004764642, 0.004507915, 0.002469953, -0.001951727];  % offset current in A  31-Aug-2022 09:11:43

HW.Grad(iDevice).PaUin2PaIout(1:4) = ([0.3294095, 0.3375252, 0.3354753, 0.3319825] - HW.Grad(iDevice).PaOffsetI) ./ 1;  % amplification in A/V  31-Aug-2022 09:11:43
...
HW.Grad(iDevice).SystemTimeDelay(1:3) = [140e-6, 160e-6, 160e-6];  % time delay of gradient amplifier in s  -2024-01-11 - coil in y direction
HW.Grad(iDevice).MaxAmpSlice = 0.025;                           % maximum gradient amplitude for slice selection in T/m
