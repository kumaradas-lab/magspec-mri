...
HW.TX(iDevice).Uout2PaUout(2) = 10^((-15.0+53.0-3.0)/20);  % 53.0 dB amplification @ 24.0 MHz, 1.0 V with switch

HW.TX(iDevice).Max.PaUout(2) = 250;  % maximum peak output voltage in V
HW.TX(iDevice).Def.PaUout(2) = 110;  % default PaUout(2)=50 V => 25 W

HW.TX(iDevice).Max.Amplitude = [20, 20]*1e-3;  % maximum B1+ in T
HW.TX(iDevice).Def.Amplitude = [20, 20]*1e-3;  % default B1+ in T

%% 30 mm long rf coil bronce
...
%% RX Settings
HW.RX(iDevice).VGAGainDef = HW.RX(iDevice).VGAGainMax/2;  % reduce VGA gain to avoid saturation
