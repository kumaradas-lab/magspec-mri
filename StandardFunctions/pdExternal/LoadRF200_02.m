...
HW.TX(iDevice).ExtRFSN = 2;
HW.TX(iDevice).ExtRFType = 'RF200';  % string with type identifier

HW.TX(iDevice).ChannelDef = 2;  % Default TX Channel set to Tx2

% Amplifier
HW.TX(iDevice).Uout2PaUout(2) = 10^((-15.0+53.0-3.0)/20);  % 53.0 dB amplification @ 24.0 MHz, 1.0 V amp only
...
%     HW.TX(iDevice).DampCoil.DigitalOutputDuration = 2.0e-6; % duration of signal on digital out in seconds
%     HW.TX(iDevice).DampCoil.DampingDuration = 2.0e-6; % effective duration of damping the coil in seconds
%   case {'15mm_light_single'}
%     HW.TX(iDevice).DampCoil.DigitalOutputDuration = 2.4e-6; % duration of signal on digital out in seconds
...
%     HW.TX(iDevice).DampCoil.DigitalOutputDuration = 1.4e-6; % duration of signal on digital out in seconds
%     HW.TX(iDevice).DampCoil.DampingDuration = HW.TX(iDevice).DampCoil.DigitalOutputDuration+2e-6; % effective duration of damping the coil in seconds
%   otherwise
%     HW.TX(iDevice).DampCoil.DigitalOutputDuration = 1.4e-6; % duration of signal on digital out in seconds
%     HW.TX(iDevice).DampCoil.DampingDuration = 1.4e-6; % effective duration of damping the coil in seconds
% end
...
if HW.TX(iDevice).DampCoil.Enable
  ...
  HW.TX(iDevice).BlankPostsetAQ = 6000e-9;
end
...
% switch HW.UserName
%   case {'10mm_single', '10mm_both'}
%     HW.RX(iDevice).LNAGain=10^(((-51.7175)-(-73.75))/20);  % 22.0325 dB gain @ 24300494.5011 MHz F=1.4714 dB (-73.75 dBm cal)
%   case {'15mm_single', '15mm_both', '14.8mm_light_single'}
%     HW.RX(iDevice).LNAGain=10^(((-51.3859)-(-73.54))/20);  % 22.1541 dB gain @ 21500436.5131 MHz F=1.6048 dB (-73.54 dBm cal) 21.50+22.15+1.60
%   case {'15mm_light_single'}
%     HW.RX(iDevice).LNAGain=10^(((-49.3502)-(-70.8655))/20);  % 21.5153 dB gain @ 21490363.0437 MHz F=1.4506 dB (-70.8655 dBm cal) with L/4 switch % in LoadTxRxLNA_50;
%   case {'15mm_light_single_b1', '15mm_light_single_b1_DC600', 'teach'}
%     HW.RX(iDevice).LNAGain=10^(((-49.3502)-(-70.8655))/20);  % 21.5153 dB gain @ 21490363.0437 MHz F=1.4506 dB (-70.8655 dBm cal) with L/4 switch % in LoadTxRxLNA_50;
%   otherwise
%     error('PD:LoadRF200:UnknownUser', ...
%       'No settings for user "%s" specified in "%s"', HW.UserName, mfilename());
%     % center frequency: HW.RX(iDevice).LNAGain=10^(((-51.5027)-(-73.67))/20); % 22.1673 dB gain @ 23200472.8774 MHz F=1.5127 dB (-73.67 dBm cal) 23.20+22.17+1.51
% end
