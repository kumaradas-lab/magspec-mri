%% Settings for rf amplifier RF-200

HW.TX.ExtRFSN = 1;
HW.TX.ExtRFType = 'RF200';  % string with type identifier

HW.TX.ChannelDef = 2;         % Default TX Channel set to Tx2

% Amplifier
HW.TX.Uout2PaUout(2) = 10^((-15.0+52.4-3.0)/20);  % 52.4 dB amplification @ 13.0 MHz, 1.0 V without switch
HW.TX.Uout2PaUout(2) = 10^((-15.0+52.4-3.0)/20);  % 52.4 dB amplification @ 13.0 MHz, 1.0 V with switch
HW.TX.Max.PaUout(2) = 250;  % maximum peak output voltage in V
HW.TX.Def.PaUout(2) = 50;  % default PaUout(2)=50 V => 25 W

HW.TX.Max.Amplitude = [20,20]*1e-3;   % maximum B1+ in T
HW.TX.Def.Amplitude = [20,20]*1e-3;   % default B1+ in T

%% Coil 10mm
HW.RX2TXdeadTime = 1e-6;         % Receiver deadtime before TX pulse in s
HW.TX2RXdeadTime = 10e-6;        % Receiver deadtime after TX pulse in s e.g. ringing of Coil ~40 µs
HW.TX.BlankOffset = 1200e-9;      % Unblank of RF-100 before TX pulse
HW.TX.BlankPostset = 400e-9;     % Blank of RF-100 after TX pulse

%% TRx switch during transmit at Tx2
HW.TX.BlankOffsetAQ = 1000e-9;   % Blank of receiver before TX pulse
if HW.TX.DampCoil.Enable
    HW.TX.BlankPostsetAQ = 1400e-9;  % Blank of receiver after TX pulse
else
    HW.TX.BlankPostsetAQ = 2400e-9;
end
HW.TX.BlankAQ = 1;               % Switch TRx to 50 Ohm resistor during TX pulse, to avoid saturation.

%% RX Settings
HW.RX.VGAGainDef = HW.RX.VGAGainMax/1; % reduce VGA gain to avoid saturation

%%
UseExtRFAmpSwitch = 0;  % use switch of RF-200
LoadExtRFAmp_Cal;  % new cal Uout and 6 A FET
