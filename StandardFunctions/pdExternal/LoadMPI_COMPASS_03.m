%% calibration properties and settings for the probe COMPASS #3
...
HW.MPI.COMPASS_CoilEfficiency = 1.27e-3;  % efficiency of the TX coil in T/A
...
HW.TX(iDevice).LoadRin(2) = 3.45/(7.92/2);  % resistance of COMPASS TX coil in Ohm
...
HW.TX(iDevice).Inductance(2) = 59e-6;  % inductance of COMPASS TX coil in Henry
...
if ~isCalibratedCH5
  ...
  I_peak_ACmeas = 7.9/2;  % [A]
  I_peak_DCmeas = 7.9/2;  % [A]
...
TXACcal=[1,   1000, 7.74/2];
TXACcal=[2,   2000, 7.75/2];
TXACcal=[3,   5000, 7.72/2];
TXACcal=[4,   7500, 7.79/2];
TXACcal=[5,  10000, 7.84/2];
TXACcal=[6,  15000, 7.93/2];
TXACcal=[7,  20000, 7.96/2];
TXACcal=[8,  25000, 7.94/2];
TXACcal=[9,  30000, 7.86/2];
TXACcal=[10, 40000, 7.61/2];
TXACcal=[11, 50000, 7.16/2];
TXACcal=[12, 60000, 6.73/2];
TXACcal=[13, 70000, 5.52/2];
TXACcal=[14, 80000, 4.99/2];
...
HW.TX(iDevice).CoilMaxTemperature = 70;  % maximum temperature of coil before damage in degrees C
...
HW.TX(iDevice).CoilPowerDissipation = 80;  % thermal power dissipation in W at maximum temperature
