function pulseData = Pulse_Rect_Saturation(HW, Center, Pulse, varargin)
%% Create series of 90 degree RF pulses to destroy longitudinal magnetization
%
%   pulseData = Pulse_Rect_Saturation(HW, Center, Pulse)
% additionally:
%   excitationAngleFactor = Pulse_Rect_Saturation(HW, 'Amp')
%   bandwidthFactor = Pulse_Rect_Saturation(HW, 'Time')
%
...
%
% This creates a simple series of 90° pulses (no spoilers).
% See, e.g.: Phys. Chem. Chem. Phys., 2013, 15, 7123-7132, Fig. 1 (A)
% https://pubs.rsc.org/en/content/articlehtml/2013/cp/c3cp44379e
%
%
% INPUT:
%
...
%   Center
%
%       The center of the last pulse in the series in seconds (tRep).
%
%   Pulse
...
%
%     FlipAngle
%         The total (effective) flip angle of each pulse in radians (or the
%         units defined by FlipAngleFullTurn). It is used to set an appropriate
%         pulse amplitude. (Default: pi/2)
%
%     FlipAngleFullTurn
...
%
%     MaxNumberOfSegments
%         Maximum number of segments of each pulse. (Default: 1)
%
%     MaxLength
%         Maximum length of each pulse in seconds. (Default: Inf)
%
%     Frequency
...
%
%     Bandwidth
%         Bandwidth of each pulse in Hz. (Default: max(1/Pulse.MaxLength, 2e3) )
%
%     nPulses
%         Number of excitation pulses in series. (Default: 10)
%
%     separationFactor
%         Separation between the rf pulses as a factor to the duration of each
%         rf pulse. (Default: 1)
%
%
% OUTPUT:
...
Pulse = set_EmptyField(Pulse, 'nPulses', 11);
Pulse = set_EmptyField(Pulse, 'separationFactor', 1);
...
if Pulse.MaxLength + 1/HW.TX(Pulse.iDevice).fSample < BlockLength
  error('PD:Pulse_Rect_Saturation:MaxLengthTooShort', ...
...
pulseData.Start = -BlockLength/2 + Center ...
  + ((-Pulse.nPulses+1):0).'*(BlockLength*(1+Pulse.separationFactor));
pulseData.Amplitude = gain * ones(size(pulseData.Start));
pulseData.Duration = BlockLength * ones(size(pulseData.Start));
pulseData.Frequency = Pulse.Frequency * ones(size(pulseData.Start));
