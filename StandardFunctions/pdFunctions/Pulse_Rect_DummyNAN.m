function [pulseData] = Pulse_Rect_DummyNAN(HW, Center, Pulse, varargin)
%% Create a rectangular dummy RF pulse with zero RF output amplitude
%
%   pulseData = Pulse_Rect_DummyNAN(HW, Center, Pulse)
% or:
%   pulseData = Pulse_Rect_DummyNAN(HW, Center, Bandwidth, FlipAngle, MaxNumberOfSegments,  MaxLength, Frequency, Phase)
% additionally:
%   excitationAngleFactor = Pulse_Rect_DummyNAN(HW, 'Amp')
%   bandwidthFactor = Pulse_Rect_DummyNAN(HW, 'Time')
%
% INPUT:
%   HW        HW structure
...
pulseData.Start = -BlockLength/2 + Center + NaN;
pulseData.Amplitude = gain*0;
