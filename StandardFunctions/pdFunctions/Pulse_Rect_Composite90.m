function [pulseData] = Pulse_Rect_Composite90(HW, Center, Pulse, varargin)
%% Create a composite 90 degree (excitation) RF pulse
%
...
%
% This function can be used to create excitation pulses that are composed of
...
%             of the composite pulse (default: [90; 270; 360]).
%     FlipPhaseComposite
...
Pulse = set_EmptyField(Pulse, 'FlipAngleComposite', [ 90;270;360]);
Pulse = set_EmptyField(Pulse, 'FlipPhaseComposite', [  0; 90;270]+0);
Pulse = set_EmptyField(Pulse, 'FactorAmp', sum(Pulse.FlipAngleComposite)/90);
Pulse = set_EmptyField(Pulse, 'FactorTime', sum(Pulse.FlipAngleComposite)/90);
Pulse = set_EmptyField(Pulse, 'Bandwidth', max(Pulse.FactorTime/Pulse.MaxLength, 2e3)); % FIXME: Is this a sensible default?
Pulse = set_EmptyField(Pulse, 'iDevice', 1);

if nargin == 2
  %% short path (additional syntax)
  if strcmp(Center,'Amp')
    pulseData = Pulse.FactorAmp;
  elseif strcmp(Center,'Time')
    pulseData = Pulse.FactorTime;
  else
    pulseData = NaN;
  end
  return;
...
if Pulse.MaxNumberOfSegments < numel(Pulse.FlipAngleComposite)
  error('Pulse_Rect_Composite90: MaxNumberOfSegments must be at least %d.', ...
    numel(Pulse.FlipAngleComposite));
end

pulseData.Start = (cumsum([0;Pulse.FlipAngleComposite(1:end-1)])-sum(Pulse.FlipAngleComposite)/2) / ...
  90*BlockLength + Center;
pulseData.Amplitude = ones(size(Pulse.FlipAngleComposite)) * gain;
pulseData.Duration = Pulse.FlipAngleComposite/90*BlockLength;
