...
%
%   SeqLoop
%       Structure with results of a measurement.
%   Image3D
%       3D matrix with image data.
%   FileName
%       Optional. File name for the VolView binary data.
%       (Default: 'output/VolView3D_yyyymmdd_HHMMSSFFF')
%   ZeroFillFactor
%       Optional. Scalar or 1x3 zero fill factor for padding of the k-space
%       before the FFT.
%       (Default: 1)
%   ZeroFillWindowSize
%       Optional. To reduce the "jitter" in the image, k-space frequencies
%       further away from the center frequency can be damped. A low value for
%       "ZeroFillWindowSize" leads to higher damping, a "ZeroFillWindowSize" of
%       Inf means no damping. A good value in most circumstances is 1.4 (approx.
%       sqrt(2)).
%       (Default: 1.4).
%
%
...
%
%   SeqLoop
%       The same as input "SeqLoop".
%       Or: A structure with "Image3D" and the values that where written to the
%       VolView files.
%
%
% ------------------------------------------------------------------------------
...
if isemptyfield(SeqLoop.AQSlice(1), 'VoxelVolume')
  SeqLoop.AQSlice(1).VoxelVolume = 1;
end
if isemptyfield(SeqLoop.AQSlice(1), 'AreaCoil')
  SeqLoop.AQSlice(1).AreaCoil = 1;
end
if isemptyfield(SeqLoop.AQSlice(1), 'AmplitudeUnit')
  SeqLoop.AQSlice(1).AmplitudeUnit = 'au';
end
if isemptyfield(SeqLoop.AQSlice(1), 'AmplitudeUnitScale')
  SeqLoop.AQSlice(1).AmplitudeUnitScale = 1;
end


if isemptyfield(SeqLoop.AQSlice(1), 'kLineEnd') && SeqLoop.AQSlice(1).nRead~=1
  % likely encoding in read-phase-phase and measurements with regular k-space
  regularKSpace = true;
else
  % This is likely the result of a ZTE or SPI measurement.
  regularKSpace = false;
end
...
if nargin < 3
  if isemptyfield(SeqLoop.AQSlice(1), 'ZeroFillFactor')
    SeqLoop.AQSlice(1).ZeroFillFactor = 1;
  end
  ZeroFillFactor = SeqLoop.AQSlice(1).ZeroFillFactor;
end
ZeroFillFactor = reshape(ZeroFillFactor(1:min(end,3)), 1, []);
if nargin < 4
  if isemptyfield(SeqLoop.AQSlice(1), 'ZeroFillWindowSize')
    SeqLoop.AQSlice(1).ZeroFillWindowSize = 1.4;
  end
...
if regularKSpace
  % assume encoding in read-phase-phase for measurements with regular k-space
  ...
  resolutionImage = [SeqLoop.AQSlice(1).resolutionRead(1), ...
    SeqLoop.AQSlice(1).resolutionPhase(1:2)];
else
  % This is likely the result of a ZTE measurement
  sImage = [SeqLoop.AQSlice(1).nPhase(1), SeqLoop.AQSlice(1).nPhase(2), SeqLoop.AQSlice(1).nPhase(3)];
  resolutionImage = SeqLoop.AQSlice(1).resolutionPhase(1:3);
end
...
resolutionImage = resolutionImage ./ lengthScaling ./ ZeroFillFactor;
...
elseif isfield(SeqLoop.data, 'ImageOs')
  ...
  if regularKSpace
    szImageOs = arrayfun(@(n) size(SeqLoop.data.ImageOs, n), 1:3);
  else
    szImageOs = arrayfun(@(n) size(SeqLoop.data.ImageOs, n), 2:4);
  end
  szImageOsZ = szImageOs .* ZeroFillFactor;
  szImageOsZ(szImageOs==1) = 1;
  if regularKSpace
    szImageOsZ = [szImageOsZ, 1];
  else
    szImageOsZ = [1, szImageOsZ];
  end
  ...
  if regularKSpace
    ...
    MySizeOS = [ ...
      SeqLoop.AQSlice(1).nPhase(1)*SeqLoop.AQSlice(1).PhaseOS(1), ...
      SeqLoop.AQSlice(1).nPhase(2)*SeqLoop.AQSlice(1).PhaseOS(2), ...
      SeqLoop.AQSlice(1).nPhase(3)*SeqLoop.AQSlice(1).PhaseOS(3) ] .* ZeroFillFactor;
    MySize = [ ...
      SeqLoop.AQSlice(1).nPhase(1), ...
      SeqLoop.AQSlice(1).nPhase(2), ...
      SeqLoop.AQSlice(1).nPhase(3) ] .* ZeroFillFactor;
    ImageZ = ImageOsZ(:,floor(MySizeOS(1)/2) + (1-floor(MySize(1)/2):ceil(MySize(1)/2)), ...
                        floor(MySizeOS(2)/2) + (1-floor(MySize(2)/2):ceil(MySize(2)/2)), ...
                        floor(MySizeOS(3)/2) + (1-floor(MySize(3)/2):ceil(MySize(3)/2)),:);
  end
