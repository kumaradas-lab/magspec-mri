...
if nargin<=1
    ...
    if ~isfield(ABJ,'command');ABJ.command='';end; 
%     if isempty(ABJ.command);ABJ.command='';end      
    if ~isfield(ABJ,'getWeight');ABJ.getWeight=0;end; 
    if ~isfield(ABJ,'waitForWeight');ABJ.waitForWeight=0;end; 
    if ~isfield(ABJ,'clearOldWeight');ABJ.clearOldWeight=0;end; 
    if ~isfield(ABJ,'Timeout');ABJ.Timeout=1;end; 
end
if ~isfield(ABJ,'serial');ABJ.serial=[];end; 
...
if ~isfield(ABJ,'plot');ABJ.plot=[];end; if isempty(ABJ.plot);ABJ.plot=0;end        
if ~isfield(ABJ,'close');ABJ.close=[];end; if isempty(ABJ.close);ABJ.close=0;end        
% if ~isfield(ABJ,'status');ABJ.status=[];end; if isempty(ABJ.status);ABJ.status=0;end        

if ABJ.getWeight && ~ABJ.clearOldWeight
    if get(ABJ.serial,'Timeout')~=ABJ.Timeout;set(ABJ.serial,'Timeout',ABJ.Timeout);end
    t=numel(ABJ.weight);
