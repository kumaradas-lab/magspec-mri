function [pulseData] = Pulse_Rect_Composite90_Levitt_42(HW, Center, Pulse, varargin)
%% Create a composite 90 degree RF pulse as described in Levitt et al. [42]
%
%   pulseData = Pulse_Rect_Composite90_Levitt_42(HW, Center, Pulse)
% or:
%   pulseData = Pulse_Rect_Composite90_Levitt_42(HW, Center, Bandwidth, FlipAngle, MaxNumberOfSegments, MaxLength, Frequency, Phase)
% additionally:
%   excitationAngleFactor = Pulse_Rect_Composite90_Levitt_42(HW, 'Amp')
%   bandwidthFactor = Pulse_Rect_Composite90_Levitt_42(HW, 'Time')
%
% This is the composite pulse described in reference [42] of the chapter
...
%   M. H. Levitt and R. R. Ernst, J. Magn. Reson., 1983, 55, 247.
% From Table 1 (Variable Rotation Composite 90 DEG pulses):
%   rel. B1 bandwidth: {0.58, 1.42}   (1 is nominal)
%   rel. B0 bandwidth: {-1.24, 1.24}  (0 is nominal)
%
%
% INPUT:
%
...
%% composite settings
FlipAngleComposite = [180;360;180;270; 90];
FlipPhaseComposite = [  0;180;  0;180; 90];

%% check input
if nargin == 2
  % short path (additional syntax)
  if strcmp(Center, 'Amp')
    pulseData = sum(FlipAngleComposite)/90;
